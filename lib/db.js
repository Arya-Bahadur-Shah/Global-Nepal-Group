...
/* Schema creation runs once per process and every query waits on it.
   ...
   pg_advisory_lock makes the whole setup one-at-a-time across every
   ...
   The lock is held on ONE checked-out connection, not via pool.query(),
   because an advisory lock belongs to the session that took it — a
   second query on a different pooled connection would not be holding
   it. It is session-scoped, so a process that dies mid-setup releases
   it automatically when its connection drops. */
function initialise() {
  return (async () => {
    ...
    try {
      ...
      // Best-effort: if the unlock itself fails the connection is in a
      // bad enough state that releasing it drops the lock anyway.
...
async function seedAdminFromEnv(client) {
  ...
  // Runs on the locked connection, so it is covered by the same
  // exclusion as the CREATE TABLEs above.
  await client.query(
