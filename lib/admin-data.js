...
/* Home hero: loop video list + hero copy, plus the site logo.
   `logo` is only written when a value is supplied — the admin form
   sends nothing when no new file was chosen, and overwriting with null
   there would clear the existing logo on every unrelated save. */
export async function updateHomeSettings(data) {
  return runSafe(() =>
    data.logo
      ? db.prepare(`UPDATE site_settings SET "heroSub"=?, "ctaPrimary"=?, "ctaSecondary"=?, "heroVideos"=?, logo=? WHERE id=1`)
          .run(data.heroSub, data.ctaPrimary, data.ctaSecondary, j(data.heroVideos ?? []), data.logo)
...
/* Header & Footer site settings (logo, favicon, company, contact details, footer copy) */
export async function updateHeaderFooterSettings(data) {
  return runSafe(async () => {
    const current = (await db.prepare('SELECT logo, favicon FROM site_settings WHERE id = 1').get()) || {}
    const logo = data.logo ? data.logo : current.logo
    const favicon = data.favicon ? data.favicon : current.favicon
    return db.prepare(`
      UPDATE site_settings 
      SET company=?, tagline=?, address=?, phone=?, email=?, copyright=?, logo=?, favicon=?, "footerColumns"=?
      WHERE id=1
    `).run(
      data.company ?? null,
      data.tagline ?? null,
...
export async function createProduct(data) {
  return runSafe(() =>
    db.prepare(`
      INSERT INTO products ("brandSlug", "industrialSolutionSlug", slug, name, model, "shortDescription", description, image, gallery, specs, "specSheet", "specSheetVariants", tag)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(data.brandSlug, data.industrialSolutionSlug ?? null, data.slug, data.name, data.model, data.shortDescription, data.description, data.image,
...
export async function updateProduct(id, data) {
  return runSafe(() =>
    db.prepare(`
      UPDATE products SET "brandSlug"=?, "industrialSolutionSlug"=?, slug=?, name=?, model=?, "shortDescription"=?, description=?, image=?, gallery=?, specs=?, "specSheet"=?, "specSheetVariants"=?, tag=?
      WHERE id=?
    `).run(data.brandSlug, data.industrialSolutionSlug ?? null, data.slug, data.name, data.model, data.shortDescription, data.description, data.image,
...
export async function getProductsByIndustrialSolution(solutionSlug) {
  const rows = await db.prepare('SELECT * FROM products WHERE "industrialSolutionSlug" = ? ORDER BY id DESC').all(solutionSlug)
  if (rows.length > 0) return rows.map(mapProduct)
  const all = await listProducts()
  return all.filter((p) => p.industrialSolutionSlug === solutionSlug || p.brandSlug === solutionSlug)
}
...
export async function createLead({ name, email, phone, msg, type, product }) {
  const at = new Date().toISOString()
  await db.prepare(`INSERT INTO leads (name, email, phone, msg, type, product, at) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .run(name, email, phone ?? '', msg, type === 'demo' ? 'demo' : 'contact', product ?? null, at)
  return at
}
...
/**
 * Creates a password-reset token for `email`. Any earlier unused tokens for
 * the same address are retired first so only one active link exists at a time.
 * Returns the raw (unhashed) token — only this call ever sees it.
 */
export async function createPasswordResetToken(email) {
  const addr = String(email).trim().toLowerCase()
  // Expire any existing unused tokens for this address immediately.
  await db.query(
  ...
  // Produce a 32-byte cryptographically-secure random token.
  ...
  await db.query(
    `INSERT INTO admin_password_resets (email, token_hash, expires_at)
     VALUES ($1, $2, $3)`,
    [addr, hash, expiresAt.toISOString()]
  )
...
/**
 * Looks up a reset row by its raw token. Returns the row or null.
 * Does NOT mark it used — call `markPasswordResetUsed` after changing
 * the password so the two operations stay close to each other.
 */
export async function getPasswordResetByToken(rawToken) {
...
/** Marks a reset row as consumed so it cannot be used a second time. */
