...
export function getSiteRaw() {
  return db.prepare('SELECT * FROM site_settings WHERE id = 1').get() || null
}
/* Home hero: loop video list + hero copy. */
export function updateHomeSettings(data) {
  return runSafe(() =>
...
export function listClients() {
  return db.prepare('SELECT * FROM clients ORDER BY name COLLATE NOCASE ASC').all()
}
