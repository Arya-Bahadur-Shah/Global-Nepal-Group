/* ============================================================
   INDUSTRIAL SOLUTIONS — LEVEL 3  (/industrial-solutions/[slug]/[product])
   Full product detail page under an Industrial Solution.
   Features image gallery, product description, spec table, PDF spec sheet download,
   quote request button, and related products in this solution.
   ============================================================ */
...
import {
  ...
  getProductsByIndustrialSolution, getIndustrialSolutionProduct,
} from '@/lib/content'
import { Reveal, ArrowIcon } from '@/components/ui'

/* Pre-generate static pages for every solution/product pair. */
export async function generateStaticParams() {
  const solutions = await getIndustrialSolutions()
  const params = []
  for (const sol of solutions) {
    const prods = await getProductsByIndustrialSolution(sol.slug)
    for (const p of prods) {
      params.push({ slug: sol.slug, product: p.slug })
    }
  }
  return params
}
...
export async function generateMetadata({ params }) {
  const product = await getIndustrialSolutionProduct(params.slug, params.product)
  const solution = await getIndustrialSolution(params.slug)
  return {
    title: product
      ? `${product.name} — ${solution?.name || 'Industrial Solution'} — Global Nepal Group`
      : 'Product Detail',
  }
}

export default async function IndustrialSolutionProductDetailPage({ params }) {
  const solution = await getIndustrialSolution(params.slug)
  if (!solution) notFound()
  const product = await getIndustrialSolutionProduct(params.slug, params.product)
  if (!product) notFound()

  const related = (await getProductsByIndustrialSolution(params.slug))
  ...
  return (
    <>
      ...
      <section className="bg-paper pt-[72px]">
        <div className="mx-auto max-w-content px-5 sm:px-8 py-14">
          ...
          <nav className="flex items-center gap-2 font-mono text-xs text-steel">
            <Link href="/industrial-solutions" className="hover:text-gold">Industrial Solutions</Link>
            <span>/</span>
            <Link href={`/industrial-solutions/${params.slug}`} className="hover:text-gold">{solution.name}</Link>
            <span>/</span>
          ...
          <div className="mt-8 grid lg:grid-cols-2 gap-12 items-start">
            {/* LEFT — image / gallery */}
            <Reveal>
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-mist border border-cloud shadow-sm">
                {product.image ? (
            ...
            <Reveal delay={0.06}>
              <div className="font-mono text-[11px] tracking-widest uppercase text-gold">{solution.name}</div>
              ...
              <p className="mt-5 text-ink leading-relaxed">{product.description || product.shortDescription}</p>

              {/* Spec sheet download & quote CTAs */}
      ...
      {/* Related products from the same solution */}
      {related.length > 0 && (
        <section className="bg-paper py-16">
          <div className="mx-auto max-w-content px-5 sm:px-8">
            <Reveal className="flex items-end justify-between">
              <h2 className="font-display font-bold text-ocean text-2xl">More from {solution.name}</h2>
              <Link href={`/industrial-solutions/${params.slug}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-gold hover:text-crimson">
                All {solution.name} products <ArrowIcon />
              </Link>
            </Reveal>
            <div className="mt-8 grid sm:grid-cols-3 gap-x-8 gap-y-10">
              {related.map((p) => (
                <Link key={p.slug} href={`/industrial-solutions/${params.slug}/${p.slug}`} className="group">
